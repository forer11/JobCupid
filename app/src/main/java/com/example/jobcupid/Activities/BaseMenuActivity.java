...
public class BaseMenuActivity extends AppCompatActivity {
    ...
    protected void setProfile() {
        ...
        if (appData.fireBaseAuthHandler.user != null
                && appData.fireBaseAuthHandler.user.getEmail() != null
                && !appData.fireBaseAuthHandler.user.getEmail().equals("")) {
            setProfileImageWithUrl(appData);
            appData.fireStoreHandler.createUserIfNotExists(appData.fireBaseAuthHandler.user);
        }
    }

    private void setProfileImageWithUrl(AppData appData) {
        Uri uri = appData.fireBaseAuthHandler.user.getPhotoUrl();
        if (uri != null) {
            Glide
                    .with(profileImage)
                    .load(uri.toString())
    ...
    private void onClickDialog(View view, final AlertDialog alertDialog) {
        ...
        signOut.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view1) {
                ...
                AppData appData = (AppData) getApplicationContext();
                GoogleSignInClient googleSignInClient = appData
                        .fireBaseAuthHandler
                        .googleSignInClient;
